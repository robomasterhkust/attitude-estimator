...
void mpu6050_init(uint8_t devAddr)
{
    uint8_t   i = 0;
    esp_err_t ret;
    uint8_t   MPU6050_Init_Data[5][2] =
    {
        { MPU6500_PWR_MGMT_1,   0x01 },       // wake up from sleep mode
        { MPU6500_SMPLRT_DIV,   0x07 },       // update frequency to 1kHz
        // { MPU6500_PWR_MGMT_2,     0x00 },     // Enable Acc & Gyro
        { MPU6500_CONFIG,       0x00 },       //
        { MPU6500_GYRO_CONFIG,  0x08 },       // +- dps
    ...
    for (i = 0; i < 5; i++)
    {
        ret = i2c_write_reg(I2C_MASTER_NUM, devAddr, MPU6050_Init_Data[i][0], MPU6050_Init_Data[i][1]);
        if (ret != ESP_OK)
        {
...
esp_err_t mpu6050_getMotion6(uint8_t devAddr, imu_data_t *imu_data)
{
    ...
    ret = i2c_read_words(I2C_MASTER_NUM, devAddr, MPU6500_ACCEL_XOUT_H, imu_buff, 14);
    ...
    {
        xSemaphoreTake(print_mux, portMAX_DELAY);
        printf("Error: No ack, sensor not connected\n");
        xSemaphoreGive(print_mux);
    }
    free(imu_buff);
    imu_buff = NULL;
